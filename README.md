...
The input is tokenized first into numbers and operators, then organized by operator precidence and left/right association. In this case the only right associative operation being exponents. 
...
# Features
The program can handle addition, subtraction, multiplication, divition, and exponentiation. Sections can be encapsilated in brackets for altering the order of operations. The program also supports floating point notation. While white space between tokens (numbers and operation symbols) is ignored.
```bash
> (8*(4/8)+6-7)^2
9.000000
...
```
## Size restriction.
The program by default has a max array size of 256, which is set in globals.h. Memory is prealocated in the run function, and consists of an input buffer `char input[MAX_SIZE]`, two Queues, and two Stacks. The static memory allocation in function `run` with a max size of 256 ends up being ~16.77 Kb.
...
# Usage
Type in the equasion you want the answer too and hit `Enter ↵`
## Symbols 
- Add = '+'
- Subtract = '-'
- Multiply = '*'
